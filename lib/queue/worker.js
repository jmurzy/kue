...
/**
 * Process a depromoted `job`. Set's the job's state
 * to "delayed", in which case job data will be updated
 * and the job will be put back in the delayed queue.
 *
 * @param {Function} fn
 * @return {Worker} for chaining
 * @api private
...
// JAKE#
// TODO: reset attempts
Worker.prototype.depromoted = function(job, err, fn){
  var self = this;
  events.emit(job.id, 'depromotion');
  job.depromoted();
  self.start(fn)
};
...
Worker.prototype.process = function(job, fn){
  ...
  fn(job, function(err){
    ...
    // JAKE#
    // Depromote job if processing indicates depromotion
    if (job.isDepromoted()) return self.depromoted(job, err, fn);
