...
exports.get = function(id, fn){
  ...
  client.hgetall('q:job:' + job.id, function(err, hash){
    ...
    job.depromoted_at = hash.depromoted_at;
...
Job.prototype.toJSON = function(){
  return {
    ...
    , depromoted_at: this.depromoted_at
...
// JAKE#
// virtually depromote the job object state
Job.prototype.depromote = function(ms){
  if (0 == arguments.length) return this._delay;
  this._delay = ms;
  //this._state = 'depromoted';

  var depromoted_at = Date.now();

  this.depromoted_at = depromoted_at;
  this.set('depromoted_at', depromoted_at);

 // console.log("........................#", this.depromoted_at);
...
// JAKE#
// put the depromoted job back in the delayed que
// we also update the job
Job.prototype.depromoted = function(fn){
  var self = this, 
      fn = fn || noop;

 // this.log("Job is depromoted at ", Date.now().toString(), ".")
  
  this._state = 'delayed';
  //this.update(fn)
  return this.update(fn);//this.state('delayed');
};

// // JAKE#
// // returns the current virtual/object state
// Job.prototype.getState = function(){
//   return this._state;
// }

Job.prototype.isDepromoted = function(){
//  console.log("debuggingggggg", this.depromoted_at, this.updated_at)

    return this.depromoted_at ? (this.depromoted_at > this.updated_at) : false;
}
...
Job.prototype.removeState = function(fn){
  var client = this.client
    // JAKE#
    // remove depends on the previous state to clena up some data
    // , state = this._state == "depromoted" ? "active" : this._state; 
    , state = this._state; 
...
Job.prototype.update = function(fn){
  ...
  // // JAKE#
  // // so that we can recalculate the promotion time when depromoted
